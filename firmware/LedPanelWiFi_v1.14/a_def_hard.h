...
#define HEIGHT 39               // высота одного сегмента матрицы для MATRIX_TYPE == 0 или MATRIX_TYPE == 1                                       | для MATRIX_TYPE == 2 ПОЛНАЯ ВЫСОТА матрицы 
#define MATRIX_TYPE 2           // тип соединения диодов в сегменте матрицы: 0 - зигзаг, 1 - параллельная, 2 - использовать карту индексов
#define MATRIX_INDEX 0          // только для MATRIX_TYPE == 2 - место расположения массива индексов 0 - в сектче, 1 - в файле                    | для MATRIX_TYPE == 0 или 1 - поставьте тут 1
...
#define USE_BUTTON 0            // 1 - использовать кнопку управления; 0 - не использовать 
...
#define FS_AS_SD 1              // при USE_SD == 1; FS_AS_SD == 0 - эффекты на SD-карте, 1 - эффекты в файловой ситеме МК, самого SD-card модуля нет (режим эмуляции SD-карты для эффектов 'SD-карта')
...
#define LED_PIN 4               // D2 физческий пин ленты
#define PIN_BTN 12               // D4 физическая кнопка подключена сюда (D4 --- КНОПКА --- GND) 
...
#define POWER_PIN 13            // D1 управляющий пин вкл/выкл матрицы через MOSFET / реле; POWER_ON - HIGH, POWER_OFF - LOW
...
#define STX 0//D3                  // подключен в RX пин (2) модуля DFPlayer
#define SRX 2//D4                  // подключен в TX пин (3) модуля DFPlayer
#define DIO 14//D5                  // D5 TM1637 display DIO pin - требуется для компиляции скетча (USE_TM1637 == 1) - совместная работа с SD-картой невозможна, т.к. используют те же пины - либо то, либо то
#define CLK 13//D7                  // D7 TM1637 display CLK pin - требуется для компиляции скетча (USE_TM1637 == 1) - совместная работа с SD-картой невозможна, т.к. используют те же пины - либо то, либо то
